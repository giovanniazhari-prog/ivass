...
print(">>> socketio version:", pkg_resources.get_distribution("python-socketio").version, flush=True)
...
async def kb_gacha(msg: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="1 nomor",  callback_data="gacha:1:all"),
            InlineKeyboardButton(text="3 nomor",  callback_data="gacha:3:all"),
            InlineKeyboardButton(text="5 nomor",  callback_data="gacha:5:all"),
            InlineKeyboardButton(text="10 nomor", callback_data="gacha:10:all"),
        ],
        [
            InlineKeyboardButton(text="👑 Bio+LMB saja", callback_data="gacha:5:bio_lmb"),
            InlineKeyboardButton(text="✅ Bio saja",      callback_data="gacha:5:bio"),
        ],
        [
            InlineKeyboardButton(text="🔵 LMB saja",     callback_data="gacha:5:lmb"),
            InlineKeyboardButton(text="⚪ Semua kualitas", callback_data="gacha:5:all"),
        ],
    ])
    await msg.answer(
        f"<b>🎰 Ambil Nomor Bio</b>\n{SEP}\n"
        f"📊 Stok tersedia: <b>{total} nomor</b>\n"
        f"  👑 Bio+LMB : <b>{q.get('bio_lmb',0)}</b>\n"
        f"  ✅ Bio      : <b>{q.get('bio',0)}</b>\n"
        f"  🔵 LMB      : <b>{q.get('lmb',0)}</b>\n"
        f"  ⚪ Standard : <b>{q.get('standard',0)}</b>\n"
        f"{SEP}\n"
        f"Pilih jumlah & kualitas:",
        parse_mode="HTML",
        reply_markup=kb,
    )


@router.callback_query(F.data.startswith("gacha:"))
async def cb_gacha(cb: CallbackQuery):
    ...
    quality = parts[2]      if len(parts) > 2 else "all"

    numbers = database.get_random_numbers(count=count, filter_quality=quality)
    if not numbers:
        q_name = {"bio_lmb":"Bio+LMB","bio":"Bio","lmb":"LMB","all":"semua kualitas"}.get(quality, quality)
        await cb.message.answer(
            f"⚠️ Tidak ada nomor <b>{q_name}</b> di stok!\nCoba kualitas lain.",
            parse_mode="HTML",
        )
        return

    q_icon = {"bio_lmb":"👑","bio":"✅","lmb":"🔵","standard":"⚪","all":"🎲"}.get(quality,"🎲")
    q_name = {"bio_lmb":"Bio+LMB","bio":"Bio","lmb":"LMB","standard":"Standard","all":"Mix"}.get(quality,quality)

    lines = [f"<b>{q_icon} {len(numbers)} Nomor {q_name}</b>\n{SEP}"]
    for i, (num, q) in enumerate(numbers, 1):
        iq = {"bio_lmb":"👑","bio":"✅","lmb":"🔵","standard":"⚪"}.get(q,"⚪")
        lines.append(f"{i}. <code>{num}</code> {iq}")
...
async def _monitor_loop(bot: Bot, chat_id: int, cookies_raw: str):
    """
    Background task:
    1. Ambil socket params fresh dari /portal/live/my_sms
    2. Connect socket.io ke ivasms.com:2087/livesms
    3. Listen event → forward OTP ke Telegram
    4. Keepalive ping tiap 20 menit → session tidak expired + cookies di-save
    ...
    while True:
        ...
        try:
            # Ambil cookies terbaru dari DB (mungkin sudah diperbarui keepalive)
            ...
            async with IVASMSClient(cookies_raw) as client:
                ...
                if params:
                    # Simpan cookies yang mungkin diperbarui server
                    updated = client.get_updated_cookies_str()
                    if updated:
                        database.set_setting("ivasms_cookies", updated)
                        cookies_raw = updated
